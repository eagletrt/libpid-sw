 ...
 */
 ...
 PidController_t pid_controller_prev_errors;
 ...
 void setUp(void) {
     ...
     int ret1 = pid_controller_init(&pid, 1.0f, 0.1f, 0.01f, 0.1f, 10.0f, NULL, 0);
     TEST_ASSERT_EQUAL_INT(PID_SUCCESS, ret1);
     pid.set_point = 10.0f;
     
     /* Initialize PID mode (n_prev_errors = 3) */
     int ret2 = pid_controller_init(&pid_controller_prev_errors, 1.0f, 0.1f, 0.01f, 0.1f, 10.0f, &arena_instance, 3);
     TEST_ASSERT_EQUAL_INT(PID_SUCCESS, ret2);
     pid_controller_prev_errors.set_point = 10.0f;
 }
 ...
 void tearDown(void) {
     ...
     pid_controller_reset(&pid_controller_prev_errors);
 ...
 void test_pid_update_PI_input8(void) {
     pid_controller_update(&pid, 8.0f);
 ...
 void test_pid_update_PI_input9(void) {
     pid_controller_update(&pid, 9.0f);
 ...
 void test_pid_compute_PI(void) {
     pid_controller_update(&pid, 8.0f);
     float output = pid_controller_compute(&pid);
 ...
 void test_pid_initialization_with_prev_errors(void) {
     TEST_ASSERT_EQUAL_FLOAT(1.0f, pid_controller_prev_errors.kp);
     TEST_ASSERT_EQUAL_FLOAT(0.1f, pid_controller_prev_errors.ki);
     TEST_ASSERT_EQUAL_FLOAT(0.01f, pid_controller_prev_errors.kd);
     TEST_ASSERT_EQUAL_FLOAT(0.1f, pid_controller_prev_errors.sample_time);
     TEST_ASSERT_EQUAL_FLOAT(10.0f, pid_controller_prev_errors.anti_windUp);
     TEST_ASSERT_EQUAL_FLOAT(10.0f, pid_controller_prev_errors.set_point);
     TEST_ASSERT_EQUAL_FLOAT(0.0f, pid_controller_prev_errors.integrator);
     TEST_ASSERT_EQUAL_FLOAT(0.0f, pid_controller_prev_errors.error);
     for (int i = 0; i < pid_controller_prev_errors.n_prev_errors; i++) {
         TEST_ASSERT_EQUAL_FLOAT(0.0f, pid_controller_prev_errors.prev_errors[i]);
     }
 }
 ...
 void test_pid_update_with_prev_errors_input8(void) {
     pid_controller_update(&pid_controller_prev_errors, 8.0f);
     TEST_ASSERT_EQUAL_FLOAT(2.0f, pid_controller_prev_errors.error);
     TEST_ASSERT(pid_controller_prev_errors.integrator > 0.0f);
     /* On first update, the previous error buffer should store the initial error (0.0) */
     uint8_t index = pid_controller_prev_errors.prev_error_index;
     TEST_ASSERT_EQUAL_FLOAT(0.0f, pid_controller_prev_errors.prev_errors[index]);
 }
 ...
 void test_pid_update_with_prev_errors_input9(void) {
     ...
     pid_controller_update(&pid_controller_prev_errors, 9.0f);
     TEST_ASSERT_EQUAL_FLOAT(1.0f, pid_controller_prev_errors.error);
     uint8_t index = pid_controller_prev_errors.prev_error_index;
     /* The buffer at the current index should hold the error from the previous update (2.0) */
     TEST_ASSERT_EQUAL_FLOAT(2.0f, pid_controller_prev_errors.prev_errors[index]);
 }
 ...
 void test_pid_update_with_prev_errors_input10(void) {
     ...
     pid_controller_update(&pid_controller_prev_errors, 8.0f);   // error = 2.0
     pid_controller_update(&pid_controller_prev_errors, 9.0f);   // error = 1.0
     pid_controller_update(&pid_controller_prev_errors, 10.0f);  // error = 0.0
     TEST_ASSERT_EQUAL_FLOAT(0.0f, pid_controller_prev_errors.error);
     ...
     TEST_ASSERT((pid_controller_prev_errors.prev_errors[index] == 1.0f) ||
                 (pid_controller_prev_errors.prev_errors[(index+1) % pid_controller_prev_errors.n_prev_errors] == 1.0f));
 }
 ...
 void test_pid_compute_with_prev_errors(void) {
     pid_controller_update(&pid_controller_prev_errors, 8.0f);
     float output = pid_controller_compute(&pid_controller_prev_errors);
     TEST_ASSERT(output > 0.0f);
 }
...
void test_pid_integrator_positive_clamp(void)
{
    ...
    int ret = pid_controller_init(&pid_test, 
                        1.0f,    // kp
    ...
    pid_controller_update(&pid_test, 0.0f); 
...
void test_pid_no_antiwindup_when_ki_is_small(void)
{
 ...
 void test_pid_init_null_ptr(void) {
     int ret = pid_controller_init(NULL, 1.0f, 0.1f, 0.01f, 0.1f, 10.0f, NULL, 0);
 ...
 void test_pid_init_null_arena(void) {
     int ret = pid_controller_init(&pid, 1.0f, 0.1f, 0.01f, 0.1f, 10.0f, NULL, 3);
     TEST_ASSERT_EQUAL_INT(PID_ERROR_NULL_PTR, ret);
 }
